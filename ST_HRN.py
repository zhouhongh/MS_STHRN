...
class ST_HRN(nn.Module):
    ...
    def forward(self, encoder_inputs, decoder_inputs, train):
        ...
        g_s_spine = torch.mean(h[:, :, 6:10, :], 2, keepdim = True).expand_as(h)
        g_s_left_arm = torch.mean(h[:, :, 10:14, :], 2, keepdim = True).expand_as(h)
        g_s_right_arm = torch.mean(h[:, :, 14:, :], 2, keepdim=True).expand_as(h)
        g_s_left_leg = torch.mean(h[:, :, 3:6, :], 2, keepdim=True).expand_as(h)
        g_s_right_leg = torch.mean(h[:, :, :3, :], 2, keepdim=True).expand_as(h)

        c_g_s_spine = torch.mean(c_h[:, :, 6:10, :], 2, keepdim = True).expand_as(c_h)
        c_g_s_left_arm = torch.mean(c_h[:, :, 10:14, :], 2, keepdim = True).expand_as(c_h)
        c_g_s_right_arm = torch.mean(c_h[:, :, 14:, :], 2, keepdim=True).expand_as(c_h)
        c_g_s_left_leg = torch.mean(c_h[:, :, 3:6, :], 2, keepdim=True).expand_as(c_h)
        c_g_s_right_leg = torch.mean(c_h[:, :, :3, :], 2, keepdim=True).expand_as(c_h)
...
class EncoderCell(nn.Module):
    ...
    def forward(self, h, c_h, p, g_t,c_g_t,g_t1, g_t2, g_t3,c_g_t1, c_g_t2,c_g_t3, g_s, c_g_s,g_s_spine, c_g_s_spine
        ...
        """Update g_s_spine"""
        g_ss_hat = torch.mean(h[:, :, 6:10, :], 2, keepdim=True).expand_as(h)
        ...
        """Update g_s_left_arm"""
        g_sla_hat = torch.mean(h[:, :, 10:14, :], 2, keepdim=True).expand_as(h)
        ...
        g_sra_hat = torch.mean(h[:, :, 14:, :], 2, keepdim=True).expand_as(h)
        ...
        """Update g_s_left_leg"""
        g_sll_hat = torch.mean(h[:, :, 3:6, :], 2, keepdim=True).expand_as(h)
        ...
        g_srl_hat = torch.mean(h[:, :, :3, :], 2, keepdim=True).expand_as(h)
